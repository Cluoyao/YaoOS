...
SHTCTL *shtctl_init(MEMMAN *memman, unsigned char *vram, int xsize, int ysize)
{
    ...
    /* 添加map */
    ctl->map = (unsigned char *)memman_alloc_4k(memman, xsize * ysize); /* 整个图层管理就这一个map */
    if(ctl->map == 0)
    {
        /* 为啥直接把这个结构体给扔了？？？ */
...
void sheet_updown(SHEET *sht, int height)
{
    ...
    if(old > height)
    {
        ...
        if(height >= 0)
        {
            ...
            /* 刷新其上面的图层 */
            sheet_refreshmap(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, height + 1); /* 先重新做map */
